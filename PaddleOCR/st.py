...
def check_and_download_models():
    model_dirs = {
        ...
        # 'det': 'models/det/ch_PP-OCRv4_det_infer',
        'rec': 'models/rec/en_PP-OCRv4_rec_infer'
        # 'cls': 'models/cls/ch_ppstructure_mobile_v2.0_SLANet_infer'
    }
...
def load_ocr_model():
    ...
    # cls_model_dir = 'models/cls/ch_ppstructure_mobile_v2.0_SLANet_infer'
...
# Function to convert text and coordinates to markdown
def convert_text_to_markdown(text_info, filename):
    with open(filename, 'w', encoding='utf-8') as file:
        for line in text_info:
            coords, text, confidence = line
            file.write(f"- **Detected text**: {text}\n")
            file.write(f"  - **Coordinates**: {coords}\n")
...
# Function to convert sorted text to markdown preserving indentation
def convert_text_to_markdown_with_indentation(text_info, filename):
    with open(filename, 'w', encoding='utf-8') as file:
        for line in text_info:
            coords, text, confidence = line
            left_margin = int(coords[0][0] / 10)  # Adjust factor for appropriate indentation
    ...
    try:
        ...
        with st.expander("Raw OCR results"):
            ...
            for line in sorted_text_info:
                text_line = f"Detected text: {line[1]} (Confidence score: {line[2]})\n"
                st.write(text_line)
                line_text += text_line
        ...
        # Create download buttons in sidebar
...
#         # User input for file names in sidebar
#         # line_file_name = st.sidebar.text_input("Enter the file name for line-by-line text with coordinates download:", "line_text_with_coordinates.md")
#         paragraph_file_name = st.sidebar.text_input("Enter the file name for recognized text download:", "recognized_text.md")
#         # code_file_name = st.sidebar.text_input("Enter the file name for the code snippet download:", "code_snippet.md")
        
#         # Create download buttons in sidebar
#         # if st.sidebar.button("Generate line-by-line text as markdown"):
#         #     with st.spinner('Processing...'):
#         #         with NamedTemporaryFile(delete=False, suffix=".md") as tmp_file:
#         #             convert_text_to_markdown(sorted_text_info, tmp_file.name)
#         #             tmp_file.flush()
#         #             tmp_file.seek(0)
#         #             href = create_download_link(tmp_file.name, line_file_name)
#         #             st.sidebar.markdown(href, unsafe_allow_html=True)
...
#         # if st.sidebar.button("Generate code snippet as markdown"):
#         #     with st.spinner('Processing...'):
#         #         with NamedTemporaryFile(delete=False, suffix=".md") as tmp_file:
#         #             convert_text_to_markdown_with_indentation(sorted_text_info, tmp_file.name)
#         #             tmp_file.flush()
#         #             tmp_file.seek(0)
#         #             href = create_download_link(tmp_file.name, code_file_name)
#         #             st.sidebar.markdown(href, unsafe_allow_html=True)
